...
class MaxentClassifier(ClassifierI):
    ...
    def labels(self):                                               #melabelkanDATA
        return self._encoding.labels()


    def set_weights(self, new_weights):                             #MenginisialisasiBobotKata
        """
    ...
    def weights(self):                                              #FiturBobotKata
        """
    ...
    def show_most_informative_features(self, n=10, show='all'):                         #MenunjukkanFituryangBerpengaruh
        """
        ...
        fids = sorted(list(range(len(self._weights))),
                      ...
                      reverse=True)
        if show == 'pos':
            fids = [fid for fid in fids if self._weights[fid] > 0]                      #LebihDari0 Positive
        elif show == 'neg':
            fids = [fid for fid in fids if self._weights[fid] < 0]                      #KurangDari0 Negative
        for fid in fids[:n]:
...
class BinaryMaxentFeatureEncoding(MaxentFeatureEncodingI):
    ...
    def encode(self, featureset, label):
        ...
        for fname, fval in featureset.items():                                      #Mengkonversikan input fitur menjadi joint fitur
...
def train_maxent_classifier_with_iis(train_toks, trace=3, encoding=None,
    ...
    # Count how many times each feature occurs in the training data.
    empirical_ffreq = (calculate_empirical_fcount(train_toks, encoding) /                           #Perhitungan TF
                       len(train_toks))
...
def calculate_deltas(train_toks, classifier, unattested, ffreq_empirical,                               #Menghitung Nilai Delta
                     nfmap, nfarray, nftranspose, encoding):
    """
    ...
    *delta[i]*.  In particular, it starts with a guess of
    ...
    for rangenum in range(MAX_NEWTON):
        ...
        exp_nf_delta = 2 ** numpy.sqrt(nf_delta)
